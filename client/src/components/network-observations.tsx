...
export function NetworkObservations() {
  ...
  function getRadioType(network: any): 'wifi' | 'cell' | 'bluetooth' | 'ble' {
    ...
    if (
      ssid &&
      (/bluetooth|bt|headphone|speaker|mouse|keyboard/i.test(ssid) || encryption?.includes('BT'))
    ) {
      return 'bluetooth';
    }
    ...
    if (
      ...
      (ssid &&
        /echo|dot|alexa|dell|hp|macbook|fitbit|tile|beacon|ble|jlab|airpods|microsoft/i.test(
          ssid
        )) ||
      frequency === 0 ||
            ...
            <div className='flex items-center gap-3'>
              ...
              <div>
                ...
                <p className='text-sm text-muted-foreground mt-1'>
                  All detected radio signals: WiFi, Cellular, Bluetooth
                </p>
              </div>
            </div>
            ...
            {!isConnected ? (
              <div className='text-center py-12'>
                ...
                <p className='text-muted-foreground mb-6 max-w-md mx-auto'>
                  Network observations will appear here once you restore your PostgreSQL database
                  backup with PostGIS extension.
                </p>
              ...
              <div className='space-y-4' data-testid='networks-list'>
                      ...
                      <div className='flex items-center justify-between mb-2'>
                        <div className='flex items-center gap-2'>
                          <div className='icon-container w-6 h-6 mr-2'>
                            ...
                            {radioType === 'bluetooth' && (
                              <Bluetooth className='h-3 w-3 text-purple-300' />
                            )}
                            {radioType === 'ble' && <Radio className='h-3 w-3 text-purple-300' />}
                          </div>
                          <h5 className='font-medium'>{network.ssid || 'Hidden Network'}</h5>
                      ...
                      <div className='grid grid-cols-2 md:grid-cols-4 gap-2 text-xs'>
                        ...
                        <div className='text-muted-foreground'>
                          ...
                          <span className='text-foreground font-mono'>
                            {network.signal_strength !== undefined &&
                        ...
                        <div className='text-muted-foreground'>
                          ...
                          {(() => {
                            ...
                            const SecurityIcon =
                              secInfo.level === 'high'
                                ? ShieldCheck
                                : secInfo.level === 'medium'
                                  ? Shield
                                  : ShieldAlert;
                            return (
                              <span className='flex items-center gap-1'>
                                <SecurityIcon
                                  className={`h-3 w-3 ${getSecurityLevelColor(secInfo.level)}`}
                                />
                                <span
                                  className={`${getSecurityLevelColor(secInfo.level)} font-medium`}
                                >
                                  {secInfo.short}
                                </span>
                              </span>
                ...
                <div className='text-center text-sm text-muted-foreground'>
                  Showing {filteredAndSortedNetworks.length} of {networks.data.length} total
                  observations
                  {activeFilterCount < totalFilterCount && (
                    <span className='ml-2 text-primary'>(filtered by radio type)</span>
                  )}
                </div>
              </div>
            ) : (
              <div className='text-center py-12'>
                ...
                <p className='text-muted-foreground mb-4'>
                  No radio signals have been detected yet. Data will appear here as observations are
                  collected.
                </p>
