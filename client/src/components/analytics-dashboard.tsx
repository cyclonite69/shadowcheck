    ...
    <div className='space-y-6'>
      ...
      <Card className='premium-card'>
        <CardHeader>
          ...
          <CardDescription>Real-time insights from wireless network forensics data</CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
        ...
        <Card className='premium-card'>
          <CardHeader>
            ...
            <CardDescription>RSSI analysis across detected networks</CardDescription>
          </CardHeader>
          <CardContent>
            <div className='space-y-4'>
              {signalData.length > 0 ? (
                signalData.map((range: SignalRange) => (
        ...
        <Card className='premium-card'>
          <CardHeader>
            ...
            <CardDescription>Encryption protocols and security status</CardDescription>
          </CardHeader>
          <CardContent>
            <div className='space-y-4'>
              {securityData.length > 0 ? (
                securityData.map((security: SecurityData) => {
      ...
      <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
        <Card className='premium-card'>
          ...
          <CardContent>
            <div className='text-xl font-bold'>{overview.mostActiveBand || '2.4 GHz'}</div>
            <div className='text-xs text-muted-foreground'>
        ...
        <Card className='premium-card'>
          ...
          <CardContent>
            <div className='text-xl font-bold'>{overview.detectionRate || '42'}/min</div>
            <div className='text-xs text-muted-foreground'>New network observations</div>
          </CardContent>
        </Card>
        ...
        <Card className='premium-card'>
          ...
          <CardContent>
            <div className='text-xl font-bold'>{overview.coverageArea || '2.3'} km²</div>
            <div className='text-xs text-muted-foreground'>Estimated surveillance range</div>
          </CardContent>
        </Card>
      </div>
      ...
      {overview.alerts && overview.alerts.length > 0 && (
        <div className='space-y-2'>
          {overview.alerts.map((alert: Alert, index: number) => (
            <Alert key={index} className='border-amber-500/30 bg-amber-500/5'>
              <AlertDescription className='text-amber-400'>{alert.message}</AlertDescription>
            </Alert>
          ))}
        </div>
      )}
