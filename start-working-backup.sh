...
# Configurable vars (NEW: for alignment with compose)
DB_USER="${DB_USER:-shadowcheck_user}"  # Adjust if data uses 'shadowcheck'
FRONTEND_PORT="${FRONTEND_PORT:-5173}"  # Matches compose exposure
...
wait_for_http() {
  ...
  while [[ ${elapsed} -lt ${timeout} ]]; do
    if curl -sf "${url}" >/dev/null 2>&1 || wget --quiet --tries=1 --spider "${url}" >/dev/null 2>&1; then  # NEW: Curl primary, wget fallback
...
wait_for_http "http://localhost:${FRONTEND_PORT}" "Frontend" 30 || log_warning "Frontend health check failed"
echo ""
