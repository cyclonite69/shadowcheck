...
router.get('/', async (req, res) => {
  try {
    ...
    const countsQueries = await Promise.allSettled([
      ...
      query(
        "SELECT COUNT(DISTINCT bssid)::int AS wifi FROM app.location_details_enriched WHERE radio_short ILIKE '%wifi%' OR radio_short = 'W'"
      ),

      // Bluetooth networks
      query(
        "SELECT COUNT(DISTINCT bssid)::int AS bt FROM app.location_details_enriched WHERE radio_short = 'BT'"
      ),

      // Cellular networks (radio_short LIKE 'Cell%')
      query(
        "SELECT COUNT(DISTINCT bssid)::int AS cell FROM app.location_details_enriched WHERE radio_short LIKE 'Cell%'"
      ),
    ]);

    // Extract counts safely
    const networks =
      countsQueries[0].status === 'fulfilled' ? (countsQueries[0].value.rows[0]?.networks ?? 0) : 0;
    const locations =
      countsQueries[1].status === 'fulfilled'
        ? (countsQueries[1].value.rows[0]?.locations ?? 0)
        : 0;
    const wifi =
      countsQueries[2].status === 'fulfilled' ? (countsQueries[2].value.rows[0]?.wifi ?? 0) : 0;
    const bt =
      countsQueries[3].status === 'fulfilled' ? (countsQueries[3].value.rows[0]?.bt ?? 0) : 0;
    const cell =
      countsQueries[4].status === 'fulfilled' ? (countsQueries[4].value.rows[0]?.cell ?? 0) : 0;
    ...
    const securityCounts = securityResult.rows.reduce(
      (acc, row) => {
