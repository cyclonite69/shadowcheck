 ...
 */
const calculateChannel = (frequency) => {
...
app.get("/api/v1/networks", async (req, res) => {
    ...
    try {
      ...
      `;
        ...
        if (hasBBox(req.query)) {
            params.push(Number(req.query.minLat), Number(req.query.maxLat), Number(req.query.minLon), Number(req.query.maxLon));
            where.push(`l.lat BETWEEN $${params.length - 3} AND $${params.length - 2}
        ...
        res.json({
            ...
            data: {
                ...
                summary: {
                    ...
                    security_score: Math.round(((categories[SecurityStrength.EXCELLENT] * 100 +
...
app.get("/api/v1/timeline", async (req, res) => {
    try {
        ...
        let granularity = req.query.granularity || 'auto';
