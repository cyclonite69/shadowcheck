...
router.get("/", async (_req, res) => {
    try {
        ...
        const who = await query("SELECT current_user");
        const db = await query("SELECT current_database()");
        res.json({
            ok: true,
            status: "ok",
...
router.get("/detailed", async (_req, res) => {
    ...
    try {
        ...
        const result = await client.query("SELECT PostGIS_version() as version, NOW() as timestamp");
