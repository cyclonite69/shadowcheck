...
import { registerSurveillanceRoutes } from './routes/surveillance';
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/v1/visualize', async (req, res) => {
    ...
    try {
      // For large datasets, default to reasonable limit for performance
      const limit = parseInt(req.query.limit as string) || 1000;

      const networks = await storage.getNetworks(limit);

      // Format for Kepler.gl and Mapbox visualization
      const geojson = {
        ...
        features: networks
          ...
          .map(network => ({
            ...
            geometry: {
              type: 'Point',
              coordinates: [
                parseFloat(network.longitude || '0'),
                parseFloat(network.latitude || '0'),
              ],
            },
            properties: {
              id: network.id,
              ...
              bssid: network.bssid,
              radio_type: network.radio_type || 'wifi',
              signal_strength: network.signal_strength || -100,
              security_level: network.security_level || 'unknown',
      ...
      res.json({
        ...
        total_available: await storage.getTotalNetworkCount(),
        limit,
      });
    } catch (error) {
  ...
  // Register surveillance detection routes
