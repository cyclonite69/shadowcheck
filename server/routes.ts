...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/v1/within', async (req, res) => {
    ...
    if (!isConnected) {
      return res.status(501).json({
        ok: false,
        error:
          'Database not connected. Please restore your PostgreSQL backup with PostGIS extension.',
        code: 'DB_NOT_CONNECTED',
      });
    }
    ...
    try {
      const networks = await storage.getNetworksWithin(
        latitude,
        longitude,
        radiusMeters,
        maxResults
        ...
        features: networks
          ...
          .map((network) => ({
            ...
            geometry: {
              type: 'Point',
              coordinates: [
                parseFloat(String(network.longitude || '0')),
                parseFloat(String(network.latitude || '0')),
              ],
            },
            properties: {
              id: network.id,
  ...
  app.get('/api/v1/analytics-legacy', async (req, res) => {
    const isConnected = await storage.isDatabaseConnected();
    if (!isConnected) {
      return res.status(501).json({
        ok: false,
  ...
  // Signal strength distribution endpoint
  app.get('/api/v1/signal-strength', async (req, res) => {
    ...
    try {
      ...
      res.status(500).json({
        ok: false,
        error: 'Failed to fetch signal strength distribution',
      });
    }
  });
  ...
  // Locations by BSSID endpoint
  app.get('/api/v1/locations/:bssid', async (req, res) => {
    const isConnected = await storage.isDatabaseConnected();
    if (!isConnected) {
      return res.status(501).json({
        ok: false,
    ...
    try {
      ...
      const locations = await storage.getLocationsByBssid(bssid);
      ...
      console.error('Error fetching locations by BSSID:', error);
      res.status(500).json({
        ok: false,
        error: 'Failed to fetch location data by BSSID',
      });
    }
  });
  ...
  app.get('/api/v1/radio-stats', async (req, res) => {
    ...
    try {
      // Provide radio statistics by type
      const radioStats = [
        { radio_type: 'wifi', total_observations: 0, distinct_networks: 0 },
        { radio_type: 'cellular', total_observations: 0, distinct_networks: 0 },
        { radio_type: 'bluetooth', total_observations: 0, distinct_networks: 0 },
        { radio_type: 'ble', total_observations: 0, distinct_networks: 0 }
      ];
