...
export class DatabaseStorage {
    ...
    async getLocationVisits(limit = 50) {
        const dbInstance = await getDb();
        if (!dbInstance)
            return [];
        try {
    ...
    async getNetworkPatterns(limit = 50) {
        const dbInstance = await getDb();
        if (!dbInstance)
            return [];
        try {
    ...
    async getHomeFollowingThreats(homeRadius = 100, minDistance = 500, limit = 20) {
        const dbInstance = await getDb();
        if (!dbInstance)
            return [];
        try {
    ...
    async getNetworkTimeline(bssid) {
        const dbInstance = await getDb();
        if (!dbInstance)
...
export class MemStorage {
    ...
    async getLocationVisits(limit) {
        return [];
    }
    async getNetworkPatterns(limit) {
        return [];
    }
    async getHomeFollowingThreats(homeRadius, minDistance, limit) {
        return [];
    }
    async getNetworkTimeline(bssid) {
        return { observations: [], stats: {} };
