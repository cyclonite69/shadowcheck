 ...
 */
router.get("/wifi/threats", async (req, res) => {
    ...
    `, [minDistanceKm, homeRadiusM, minHomeSightings, limit]);
        ...
        const threatsWithObservations = await Promise.all(threatsResult.rows.map(async (threat) => {
 ...
 */
router.get("/wifi/summary", async (req, res) => {
    try {
        const pool = getPool();
        const minDistanceKm = parseFloat(String(req.query.min_distance_km || '0.5'));
...
router.post("/settings", async (req, res) => {
    try {
        const pool = getPool();
        const { radio_type, min_distance_km, max_distance_km, home_radius_m, min_home_sightings, min_away_sightings, confidence_threshold, threat_level_enabled } = req.body;
...
router.post("/feedback", async (req, res) => {
    try {
        const pool = getPool();
        const { bssid, ssid, threat_level, detected_distance_km, user_rating, user_notes, whitelist_network } = req.body;
        // Validate required fields
 ...
 */
router.post("/learning/adjust", async (_req, res) => {
    try {
        const pool = getPool();
