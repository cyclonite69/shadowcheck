...
router.get('/', async (req, res) => {
    try {
        ...
        const requestedColumns = req.query.columns
            ? String(req.query.columns).split(',').map((c) => c.trim()).filter((c) => ALLOWED_COLUMNS.has(c))
            : [];
        ...
        if (req.query.radio_types) {
            const radioTypes = String(req.query.radio_types).split(',').map((t) => t.trim());
        ...
        if (req.query.data_quality) {
            const qualityLevels = String(req.query.data_quality).split(',').map((q) => q.trim());
        ...
        if (cachedTotalCount !== null && (now - cacheTimestamp) < CACHE_TTL_MS) {
            ...
            const countResult = await query(`SELECT COUNT(*) as total FROM app.api_networks ${whereClause}`, whereClause ? queryParams : []);
 ...
 */
router.get('/:mac/observations', async (req, res) => {
    try {
        const macAddress = req.params.mac;
        const limit = Math.min(Number(req.query.limit) || 1000, 10000);
        const offset = Number(req.query.offset) || 0;
        // Get total count for this BSSID
        const countResult = await query('SELECT COUNT(*) as total FROM app.locations_legacy WHERE bssid = $1', [macAddress]);
      ...
      LIMIT $2 OFFSET $3`, [macAddress, limit, offset]);
        const hasMore = offset + result.rows.length < totalCount;
        res.json({
            ok: true,
            data: result.rows,
...
router.get('/:id', async (req, res) => {
    try {
        ...
        const result = await query('SELECT * FROM app.api_networks WHERE access_point_id = $1', [accessPointId]);
