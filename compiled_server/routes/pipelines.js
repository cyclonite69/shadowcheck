...
router.get('/kml/files', async (_req, res) => {
    ...
    `);
        const importedFiles = new Map(result.rows.map(row => [row.kml_filename, parseInt(row.observation_count)]));
...
router.post('/kml/import', async (req, res) => {
    try {
        ...
        const { stdout, stderr } = await execAsync(`python3 "${parserPath}" "${kmlPath}"`, { env, timeout: 60000 });
    ...
    try {
        ...
        for (const filename of kmlFiles) {
            try {
                ...
                const { stdout } = await execAsync(`python3 "${parserPath}" "${kmlPath}"`, { env, timeout: 60000 });
                const lines = stdout.trim().split('\n');
                const jsonLine = lines[lines.length - 1];
...
router.post('/wigle/import', async (req, res) => {
    try {
        ...
        const { stdout, stderr } = await execAsync(`python3 "${parserPath}" "${wiglePath}"`, { env, timeout: 600000 } // 10 minute timeout for large databases
        );
        if (stderr) {
...
router.get('/kismet/files', async (_req, res) => {
    ...
    `);
        const importedFiles = new Map(result.rows.map(row => [row.kismet_filename, parseInt(row.device_count)]));
...
router.post('/kismet/import', async (req, res) => {
    try {
        ...
        const { stdout, stderr } = await execAsync(`python3 "${parserPath}" "${kismetPath}" ${args}`, { env, timeout: 600000 } // 10 minute timeout
        );
        if (stderr) {
...
router.post('/wigle-api/query', async (req, res) => {
    try {
        ...
        if (latrange1 !== undefined)
            params.append('latrange1', String(latrange1));
        if (latrange2 !== undefined)
            params.append('latrange2', String(latrange2));
        if (longrange1 !== undefined)
            params.append('longrange1', String(longrange1));
        if (longrange2 !== undefined)
            params.append('longrange2', String(longrange2));
